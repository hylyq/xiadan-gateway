...
**查询假数据防御**：Ctrl+C 前清空剪贴板（复制失败时不读到上次任务残留数据）；复制后按特征列验证（持仓=`成本价`+`股票余额`、成交=`成交时间`+`成交编号`、委托=`委托编号`）——非交易时段/服务器异常时树节点点击不触发页面切换，复制到的是其他查询表，验证失败重试一次，仍失败显式报错（`INTERNAL_ERROR`），绝不静默返回假数据。
