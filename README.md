...
点击"买入价格"标签（cid=1400）触发券商服务器请求（非本地下拉菜单）。策略：点击后 `sleep(0.3)` 先检测弹窗（服务器拒绝时弹窗 <0.5s 即出现）→ 有弹窗直接分类报错 → 无弹窗再用 `poll_until` 等标签变化（超时 3s，最多 2 次重试）。两种失败场景分别处理：
...
ESC×5 重置不改变价格模式，每次切换前先读标签判断当前状态。
