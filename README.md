...
- **匹配**：归一化互相关 (NCC)。将 28×38=1064 个像素视为 1064 维向量，归一化后每个模板长度为 1，NCC = 两向量点积 = cos(夹角)。夹角越小越相似，与亮度/对比度无关。NumPy C 底层实现，<5ms 完成 1,200+ 次比对。

  ```
  输入数字 '5' 与模板库逐一比对:

    模板0: 0.12   模板1: -0.05   ...   模板5₁: 0.91 ✓  模板5₂: 0.87
                                            ↑ 夹角最小 → 识别为 5
  
  NCC ∈ [-1, 1]:  1=完全相同  0=无关  -1=黑白颠倒
  置信度 < 0.25 → 该数字可能未覆盖，返回空触发重试
  ```
...
| 引擎 | 内存 | 速度 | 原理 | 角色 |
|------|------|------|------|------|
| 轻量模板匹配 | < 5MB | < 5ms | NCC 模板匹配 | 生产模式唯一引擎 |
