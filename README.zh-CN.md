...
**导航与验证码双兜底**（2026-09-05）：① 树节点点击后校验 `is_selected()` 选中态，未生效自动重试——实测模拟盘 `click_input` 偶发未注册，页面停留"当日委托"，而空表会绕过特征列验证、持仓被静默返回空列表，此修复堵住该路径；② Ctrl+C 后未检测到验证码弹窗时兜底校验剪贴板——无验证码会话/弹窗漏检时有效表格直接采用，不再空转两轮后误报 OCR 失败。
