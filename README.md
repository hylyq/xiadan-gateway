...
6. **下单耗时优化**：通过复用 UIA 控件树遍历、跳过无变化步骤、连续同向跳过准备操作等手段，单笔限价买入从 ~13.7s 降至 ~8.0s（冷启动）/~5.7s（连续同向），详见 [核心特性](#核心特性)「连续同向跳过」
...
| 连续同向跳过 | 买入→买入（或卖出→卖出）跳过窗口重置/激活/F1/F2。**仅当上笔无弹窗**或**价格超限干净退出**（点「否」正常关闭，窗口状态可信）。其他失败自动回退完整准备 |
